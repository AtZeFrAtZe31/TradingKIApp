...
# 0) Basics & Storage
# -----------------------------
say "[0/9] Speicherzugriff verknüpfen … (bei Nachfrage 'y' drücken)"
termux-setup-storage || true

say "[0/9] Pakete aktualisieren & Tools installieren …"
pkg update -y
pkg install -y openjdk-17 unzip wget git curl

# Gradle: wir nutzen den Wrapper – System-Gradle ist optional
command -v gradle >/dev/null 2>&1 || pkg install -y gradle || true

# Java 17 erzwingen (auch wenn 21 installiert ist)
if [ -d "$PREFIX/lib/jvm/openjdk-17" ]; then
  ...
  echo "[!] JDK 17 nicht gefunden. 'pkg install openjdk-17 -y' fehlgeschlagen?"
  exit 1
fi
export PATH="$JAVA_HOME/bin:$PATH"
java -version >/dev/null || { echo "[!] Java 17 läuft nicht."; exit 1; }
...
say "[1/9] Android commandline-tools installieren …"
cd "$ANDROID_SDK_ROOT"
...
say "[2/9] SDK-Komponenten + Lizenzen (kann dauern) …"
...
# 2) Projekt erzeugen
# -----------------------------
APPDIR="$HOME/TradingKIApp"
say "[3/9] Projektstruktur neu anlegen in $APPDIR …"
rm -rf "$APPDIR"
mkdir -p "$APPDIR/app/src/main/java/com/tradingki/app"
mkdir -p "$APPDIR/app/src/main/res/layout"
mkdir -p "$APPDIR/app/src/main/res/values"
mkdir -p "$APPDIR/app/src/main/assets"
...
# root build.gradle (AGP-Version hier definieren)
...
dependencies {
  ...
  // TFLite (optional – läuft weiter ohne Modell)
...
# Manifest (ohne package-Attribut)
...
# Layout mit Chart + Spinner + Toggle + Button
...
# Werte
cat > "$APPDIR/app/src/main/res/values/arrays.xml" <<'EOF'
<resources>
...
# Demo-CSV
cat > "$APPDIR/app/src/main/assets/btc_demo.csv" <<'EOF'
ts,price
0,30000
1,30010
2,30008
...
# Leeres Modell (optional)
: > "$APPDIR/app/src/main/assets/model.tflite"

# Java: DataFeed
cat > "$APPDIR/app/src/main/java/com/tradingki/app/DataFeed.java" <<'EOF'
...
# Java: Strategien
cat > "$APPDIR/app/src/main/java/com/tradingki/app/Strategies.java" <<'EOF'
package com.tradingki.app;
...
public class Strategies {
  public static float emaSignal(List<Float> prices, int fast, int slow){
    ...
    return Math.signum(emaF-emaS); // >0 BUY, <0 SELL
  }
...
# Java: TFLite Helper (optional)
cat > "$APPDIR/app/src/main/java/com/tradingki/app/TFLiteHelper.java" <<'EOF'
package com.tradingki.app;
...
# Java: MainActivity
cat > "$APPDIR/app/src/main/java/com/tradingki/app/MainActivity.java" <<'EOF'
package com.tradingki.app;
...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    Spinner strat = findViewById(R.id.strategySpinner);
    ...
    if (!ki) Toast.makeText(this, "KI-Modell nicht gefunden – läuft ohne TFLite.", Toast.LENGTH_SHORT).show();
    ...
    start.setOnClickListener(v -> {
      running = !running;
      Toast.makeText(this, live?"LIVE (Stub)":"DEMO", Toast.LENGTH_SHORT).show();
      if (running) startLoop(strat);
    });

    renderChart();
  }

  private void startLoop(Spinner strat){
  ...
  private float computeSignal(int idx){
    ...
    switch (idx){
      ...
      case 4:
        float x = w.get(w.size()-1);
        return Math.signum(tfl.predict(x));
      default: return 0f;
    }
  }

  private float tickDemo(){
    float last = prices.get(prices.size()-1);
    float drift = (rnd.nextFloat()-0.5f) * 10f;
    return last + drift;
  }

  private float tickLive(){
    float last = prices.get(prices.size()-1);
    float j = (rnd.nextFloat()-0.5f) * 15f;
    return last + j;
  }
...
# -----------------------------
# 3) Gradle Wrapper 8.5 + Build
# -----------------------------
say "[4/9] Gradle Wrapper 8.5 erstellen …"
cd "$APPDIR"
# einmalig system-gradle benutzen, danach nur noch Wrapper
gradle wrapper --gradle-version 8.5

say "[5/9] Build (assembleDebug) – Erstlauf lädt Dependencies …"
./gradlew :app:assembleDebug

# -----------------------------
# 4) APK ausliefern
# -----------------------------
say "[6/9] APK in internen Speicher kopieren …"
APK_SRC="$APPDIR/app/build/outputs/apk/debug/app-debug.apk"
APK_DST="$HOME/storage/shared/TradingKIApp.apk"
cp "$APK_SRC" "$APK_DST"

say "[✓] Fertig! APK: $APK_DST"
say "Installiere mit: termux-open \"$APK_DST\""
